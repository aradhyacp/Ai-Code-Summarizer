    ...
    <div className="min-h-screen mt-10 px-6">
      <div className="flex flex-row justify-center gap-10">
        ...
        <section className="bg-slate-900/30 p-5 rounded-2xl border border-slate-800/60 flex flex-col w-[600px]">
          ...
          <div className="flex-1 overflow-auto border border-slate-800 rounded-lg p-4 bg-[#020409] min-h-[400px]">
            {aiSummary && aiSummary.trim() !== "" ? (
              <pre className="text-sm font-mono text-slate-200 whitespace-pre-wrap">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 py-16">
                  <div className="p-4 rounded-full bg-slate-800/30 mb-3">
                    <div className="w-6 h-6 text-slate-300 animate-spin"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-rotate-cw-icon lucide-rotate-cw"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></svg></div>
                  </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 py-16">
                <div className="p-4 rounded-full bg-slate-800/30 mb-3"></div>
                <p className="text-sm">AI Summary for your code will appear here.</p>
              </div>
            )}
          </div>

          {generateFinish && <div className="flex flex-col mt-4 gap-2">
            <div className="px-4 py-3 border-1 rounded-lg bg-[#071019] border-[#222f49] flex gap-8 text-slate-400">Model Version: <span className="text-white">{modelVersion}</span></div>
            <div className="px-4 py-3 border-2 rounded-lg bg-[#071019] border-[#222f49] flex gap-8 text-slate-400">Prompt Token Count: <span className="text-white">{promptTokenCount}</span></div>
            <div className="px-4 py-3 border-2 rounded-lg bg-[#071019] border-[#222f49] flex gap-8 text-slate-400">Candidates Token Count: <span className="text-white">{candidatesTokenCount}</span></div>
            <div className="px-4 py-3 border-2 rounded-lg bg-[#071019] border-[#222f49] flex gap-8 text-slate-400">Total Token Count: <span className="text-white">{totalTokenCount}</span></div>
            <div className="px-4 py-3 border-2 rounded-lg bg-[#071019] border-[#222f49] flex gap-8 text-slate-400">Thoughts Token Count:<span className="text-white">{thoughtsTokenCount}</span></div>
          </div>}
        </section>
      </div>
    </div>
